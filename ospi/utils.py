...
def pinocchioCoordinates(model, joint_transformations, dof, representation="quat"):
    ...
    for i in xrange(1, len(model.joints)):
        if (model.joints[i].shortname() == "JointModelFreeFlyer"):
            ...
            q[qp_idx + 3:qp_idx + 7, 0] = np.matrix(
                orderQuat(
                    alg.quaternion_from_matrix(
                        alg.euler_matrix((dof[qo_idx, 0]), dof[qo_idx + 1, 0], dof[qo_idx + 2, 0], 'rxyz')))).T
            qo_idx += 6
            qp_idx += 7
            ...
            row = tup[0]
            col = tup[1]
            q[qp_idx:qp_idx + 4, 0] = np.matrix(
                orderQuat(
                    alg.quaternion_from_matrix(
                        alg.euler_matrix((dof[qo_idx, 0]) * jt_sph[row[0], col[0]],
                                         dof[qo_idx + 1, 0] * jt_sph[row[1], col[1]],
                                         dof[qo_idx + 2, 0] * jt_sph[row[2], col[2]], 'rxyz')))).T
            qo_idx += 3
            qp_idx += 4
            ...
            q[qp_idx:qp_idx + 4, 0] = np.matrix(
                orderQuat(
                    alg.quaternion_from_matrix(
                        alg.euler_matrix((dof[qo_idx, 0]), dof[qo_idx + 1, 0], dof[qo_idx + 2, 0], 'rzyx')))).T
            qo_idx += 3
            qp_idx += 4
            ...
            qo_idx += 1
            qp_idx += 1
        elif (model.joints[i].shortname() == "JointModelRX" or model.joints[i].shortname() == "JointModelRY"
              or model.joints[i].shortname() == "JointModelRZ"):
            jt_rev = jt[i - 1]
